...
#load(file="samples_BG2019_dd_obserror_time10_ddinter_stoch.Rdata")
...
plot(list.samples[[1]]$chain1[,'dd.phi.v'],type="l",ylim=c(-0.06,0))
points(list.samples[[1]]$chain2[,'dd.phi.v'],type="l",col="red")
abline(h=0)
abline(h=-0.025)

plot(list.samples[[1]]$chain1[,'dd.phi.p'],type="l",ylim=c(-0.03,0.03))
...
dd.fledg.rate.p=0#0.004
mu.phi.p=c(0.5,qlogis(0.7))
mu.phi.v=c(0.5,qlogis(0.6))
mu.fledg.rate.p=0
mu.fledg.rate.v=2
...
N <- seq(2,500,length=n.n) #density index
# intra-species DD - juvenile predator survival fn of adult predator abundance
surv_juvP_intrasp = 1/(1+exp(-(mu.phi.p[1] + dd.phi.p* N)))
# inter-species DD - juvenile prey survival fn of adult predator abundance
surv_juvV_intersp = 1/(1+exp(-(mu.phi.v[1] + dd.phi.v* N)))
# inter-species DD - predator fecundity fn of juvenile prey abundance
fecP_intersp = exp(mu.fledg.rate.p + dd.fledg.rate.p * N)
# intra-species DD - prey fecundity fn of adult prey abundance
fecV_intrasp =exp(mu.fledg.rate.v + dd.fledg.rate.v * N)

surv_juvP_intrasp_est<-surv_juvV_intersp_est<-matrix(NA,n.simul,n.n)
fecP_intersp_est<-fecV_intrasp_est<-matrix(NA,n.simul,n.n)
for(s in 1:n.simul){
  surv_juvP_intrasp_est[s,] = 1/(1+exp(-(mean(mu.phi.rec.p.est[s,])+ mean(dd.phi.p.est[s,]) * N)))
  surv_juvV_intersp_est[s,] = 1/(1+exp(-(mean(mu.phi.rec.v.est[s,])+ mean(dd.phi.v.est[s,]) * N)))
  fecP_intersp_est[s,] = exp(mean(mu.fledg.rate.p.est[s,]) + mean(dd.fledg.rate.p.est[s,]) * N)
  fecV_intrasp_est[s,] =exp(mean(mu.fledg.rate.v.est[s,]) + mean(dd.fledg.rate.v.est[s,]) * N)
}#sim
...
for(s in 1:n.simul){
  lines(N,surv_juvP_intrasp_est[s,],type='l',lwd=3,col=viridis(11,alpha=.2)[10])
}#s
lines(N,surv_juvP_intrasp,type='l',lwd=3,col=viridis(1)[1])
lines(N,colMeans(surv_juvP_intrasp_est),type='l',lwd=3,col=viridis(11)[7])
...
for(s in 1:n.simul){
  lines(N,surv_juvV_intersp_est[s,],type='l',lwd=3,col=viridis(11,alpha=.2)[10])
}#s
lines(N,surv_juvV_intersp,type='l',lwd=3,col=viridis(1)[1])
lines(N,colMeans(surv_juvV_intersp_est),type='l',lwd=3,col=viridis(11)[7])

plot(N,fecP_intersp,type='l',lwd=3,col='blue',ylab='P fecundity',ylim=c(0,25),xlab=' Juv V abundance',main='INTER-DD\n pred fec vs juv prey abund')
for(s in 1:n.simul){
  lines(N,fecP_intersp_est[s,],type='l',lwd=3,col=viridis(11,alpha=.2)[10])
}#s
lines(N,fecP_intersp,type='l',lwd=3,col=viridis(1)[1])
lines(N,colMeans(fecP_intersp_est),type='l',lwd=3,col=viridis(11)[7])

plot(N,fecV_intrasp,type='l',lwd=3,col='blue',ylab='V fecundity',ylim=c(0,6),xlab='Adult V abundance',main='INTRA-DD\n prey fec vs ad prey abund')
for(s in 1:n.simul){
  lines(N,fecV_intrasp_est[s,],type='l',lwd=3,col=viridis(11,alpha=.2)[10])
}#s
lines(N,fecV_intrasp,type='l',lwd=3,col=viridis(1)[1])
lines(N,colMeans(fecV_intrasp_est),type='l',lwd=3,col=viridis(11)[7])
