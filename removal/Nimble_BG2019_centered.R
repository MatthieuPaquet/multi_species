...
###Adjust these logical values for simulating and fitting the desired scenario
...
#DD_INTER <- TRUE
DD_INTER <- FALSE
##time series of 10 years. When FALSE the time series is of 30 years
#TIME10 <- TRUE
TIME10 <- FALSE
SAMPLE_HIGH <- TRUE
#SAMPLE_HIGH <- FALSE
...
DDconstants  <-  list(N.rec.v.star=101, N.ad.v.star=152, N.ad.p.star=21, 
                      nyears=nyears, r.j.p=r.j.p, r.j.v=r.j.v, 
...
DDmodel$setData(list(marray.j.p=marray.j.p, N.obs.p=N.obs.p, 
                     fledg.obs.p=fledg.obs.p, marray.j.v=marray.j.v,
                     N.obs.v=N.obs.v, fledg.obs.v=fledg.obs.v))
list.base <- list(mu.fledg.rate.v = 2 - 0.005 * 152, 
                  mu.phi.p=c(0.5 - 0.01 * 21, qlogis(0.7)), 
                  mu.fledg.rate.p = 0 + 0.004 * 101, 
                  mu.phi.v=c(0.5 - 0.025 * 21, qlogis(0.6)), dd.phi.p=-0.01, 
                  dd.fledg.rate.v=-0.005, p.p=0.7, p.v=0.7, N1rec.p=N1rec.p, 
                  N1ad.p=N1ad.p, N1rec.v=N1rec.v, N1ad.v=N1ad.v)
dependencies.base <- c(c("dd.phi.p","dd.fledg.rate.v","p.p","p.v","mu.phi.p",
                         "mu.phi.v","mu.fledg.rate.p","mu.fledg.rate.v",
                         "N1rec.v","N1rec.p","N1ad.v","N1ad.p"))
if (STOCH) {
  if (DD_INTER) {
    ...
    nodesToSim  <-  DDmodel$getDependencies(c("one","sigma.phi.p","sigma.phi.v",
                                              "sigma.fledg.rate.p",
                                              "sigma.fledg.rate.v","dd.phi.v",
                                              "dd.fledg.rate.p", dependencies.base),
                                            self = F, downstream = T)
  } else {
    DDinits <- c(list(sigma.phi.p=c(0.1,0.1), sigma.phi.v=c(0.1,0.1), 
                      sigma.fledg.rate.p=0.1, sigma.fledg.rate.v=0.1), list.base)
    nodesToSim  <-  DDmodel$getDependencies(c("one","sigma.phi.p","sigma.phi.v",
                                              "sigma.fledg.rate.p",
                                              "sigma.fledg.rate.v", dependencies.base),
                                            self = F, downstream = T)
  }#ddINTER
} else {
  if (DD_INTER) {
...
for (i in 1:100) {
  ...
  list.simul[[i]] <- list(N.rec.p=cDDmodel$N.rec.p,N.ad.p=cDDmodel$N.ad.p,
                          N.p=cDDmodel$N.p, N.obs.p=cDDmodel$N.obs.p, 
                          phi.p=cDDmodel$phi.p, fledg.rate.p=cDDmodel$fledg.rate.p,
                          fledg.obs.p=cDDmodel$fledg.obs.p,
                          marray.j.p=cDDmodel$marray.j.p, N.rec.v=cDDmodel$N.rec.v,
                          N.ad.v=cDDmodel$N.ad.v,N.v=cDDmodel$N.v,
                          N.obs.v=cDDmodel$N.obs.v, phi.v=cDDmodel$phi.v, 
                          fledg.rate.v=cDDmodel$fledg.rate.v, 
                          fledg.obs.v=cDDmodel$fledg.obs.v, marray.j.v=cDDmodel$marray.j.v)
  print(i)  
}
if (TIME10) {
  if (DD_INTER) {
    if (STOCH) {
      save(list.simul, file="simul_BG2019_dd_centered_time10_ddinter_stoch.Rdata")
    } else {
      save(list.simul, file="simul_BG2019_dd_centered_time10_ddinter_nostoch.Rdata")}
  } else {
    if (STOCH) {
      save(list.simul, file="simul_BG2019_dd_centered_time10_noddinter_stoch.Rdata")}
    else {
      save(list.simul, file="simul_BG2019_dd_centered_time10_noddinter_nostoch.Rdata")
    }
  }
} else {
  if (DD_INTER) {
...
plot(1:nyears.tot, N.simul.p[40,], type='l', lwd=3, ylim=c(0,max(N.simul.p[40,],
                                                                 N.simul.v[40,],
                                                                 N.simul.obs.p[40,],
                                                                 N.simul.obs.v[40,])),
...
for (i in 1) {
  ...
  marray.j.p <- matrix(NA,(nyears-1),nyears)
  marray.j.v <- matrix(NA,(nyears-1),nyears)
  marray.j.p[1:(nyears-1),1:(nyears-1)] <- list.simul[[i]]$marray.j.p[nyears.start:(nyears.start + nyears-2),
                                                                      nyears.start:(nyears.start + nyears-2)]
  marray.j.p[,nyears] <- list.simul[[i]]$marray.j.p[nyears.start:(nyears.start + nyears-2),
                                                    (nyears.start + nyears-1):dim(list.simul[[i]]$marray.j.p)[2]]
  marray.j.v[1:(nyears-1),1:(nyears-1)] <- list.simul[[i]]$marray.j.v[nyears.start:(nyears.start + nyears-2),
                                                                      nyears.start:(nyears.start + nyears-2)]
  marray.j.v[,nyears] <- list.simul[[i]]$marray.j.v[nyears.start:(nyears.start + nyears-2),
                                                    (nyears.start + nyears-1):dim(list.simul[[i]]$marray.j.v)[2]]
...
DDconstants <- list(N.rec.v.star=101,N.ad.v.star=152,N.ad.p.star=21,nyears=nyears,
                    r.j.p=r.j.p[nyears.start:(nyears.start+nyears-2)],
                    r.j.v=r.j.v[nyears.start:(nyears.start+nyears-2)],
                    fledg.sample.v=fledg.sample.v[nyears.start:(nyears.start+nyears-1)],
                    fledg.sample.p=fledg.sample.p[nyears.start:(nyears.start+nyears-1)])
DDdata <- list(marray.j.p=marray.j.p,N.obs.p=N.obs.p,fledg.obs.p=fledg.obs.p,
               marray.j.v=marray.j.v,N.obs.v=N.obs.v,fledg.obs.v=fledg.obs.v)
list.base <- list(mu.fledg.rate.v = 2 - 0.005 * 152, 
                  mu.phi.p=c(0.5 - 0.01 * 21, qlogis(0.7)), 
                  mu.fledg.rate.p = 0 + 0.004 * 101, 
                  mu.phi.v=c(0.5 - 0.025 * 21, qlogis(0.6)), dd.phi.p=-0.01, 
...
#configure the MCMC
monitor.base <- c("dd.phi.p","dd.fledg.rate.v","N.p","N.v","N.ad.p","N.ad.v",
                  "N.rec.p","N.rec.v","p.p","p.v","fledg.rate.p","fledg.rate.v",
                  "phi.p","phi.v","mu.fledg.rate.p","mu.fledg.rate.v","mu.phi.p","mu.phi.v")
  if (STOCH) {
    DDmcmcConf  <-  configureMCMC(cDDmodelIPM,monitors=c("sigma.phi.p",
                                                         "sigma.phi.v",
                                                         "sigma.fledg.rate.p",
                                                         "sigma.fledg.rate.v",
                                                         "dd.phi.v",
...
###block samplers
  if (STOCH) {
    DDmcmcConf$removeSamplers(c('dd.phi.p','dd.phi.v','dd.fledg.rate.p',
                                'dd.fledg.rate.v','mu.fledg.rate.p',
                                'mu.fledg.rate.v','mu.phi.p','mu.phi.v',
                                'sigma.phi.p','sigma.phi.v','sigma.fledg.rate.p',
                                'sigma.fledg.rate.v'))
    ...
    DDmcmcConf$addSampler(target = c('mu.phi.v[2]','sigma.phi.v[2]'),
                          ...
                          control = list(sliceAdaptFactorInterval = 20))
                            ...
                            DDmcmcConf$removeSamplers(c('dd.phi.p','dd.phi.v',
                                                        'dd.fledg.rate.p',
                                                        'dd.fledg.rate.v',
                                                        'mu.fledg.rate.p',
                                                        'mu.fledg.rate.v',
...
if (TIME10) {
  if (TRUE_DD_INTER) {
    if (STOCH) {
      save(list.samples, file="samples_BG2019_dd_centered_time10_ddinter_stoch.Rdata")
    } else {
      save(list.samples, file="samples_BG2019_dd_centered_time10_ddinter_nostoch.Rdata")}
  } else {
    if (STOCH) {
      save(list.samples, file="samples_BG2019_dd_centered_time10_noddinter_stoch.Rdata")}
    else {
      save(list.samples, file="samples_BG2019_dd_centered_time10_noddinter_nostoch.Rdata")
    }
  }
} else {
  if (TRUE_DD_INTER) {
