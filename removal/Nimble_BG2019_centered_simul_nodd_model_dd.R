...
if(TIME10){
  ...
  nmarked<-100
  if(STOCH){
    load("simul_BG2019_dd_centered_time10_noddinter_stoch.Rdata")
  }else{
    load("simul_BG2019_dd_centered_time10_noddinter_nostoch.Rdata")
  }#stoch
}else{
  nyears<-30
  nmarked<-20
...
###initial values
list.base <- list(mu.fledg.rate.v=2-0.005*152,mu.phi.p=c(0.5-0.01*21,qlogis(0.7)),mu.fledg.rate.p=0 + 0.004 * 101,mu.phi.v=c(0.5-0.025 * 21,qlogis(0.6)),dd.phi.p=-0.01,dd.fledg.rate.v=-0.005,p.p=0.7,p.v=0.7,N1rec.p=N1rec.p,N1ad.p=N1ad.p,N1rec.v=N1rec.v,N1ad.v=N1ad.v)
if(STOCH){
  if(DD_INTER){
    DDinits <- c(list(sigma.phi.p=c(0.1,0.1),sigma.phi.v=c(0.1,0.1),sigma.fledg.rate.p=0.1,sigma.fledg.rate.v=0.1,dd.phi.v=-0.025,dd.fledg.rate.p=0.004),list.base)
}else{
    DDinits <- c(list(sigma.phi.p=c(0.1,0.1),sigma.phi.v=c(0.1,0.1),sigma.fledg.rate.p=0.1,sigma.fledg.rate.v=0.1),list.base)
}#ddINTER
}else{
  if(DD_INTER){
...
if(TIME10){
  if(STOCH){
    save(list.samples,file="samples_BG2019_dd_centered_time10_sim_nodd_ddinter_stoch.Rdata")
  }else{
    save(list.samples,file="samples_BG2019_dd_centered_time10_sim_nodd_ddinter_nostoch.Rdata")
  }#stoch
}else{
  if(STOCH){
    save(list.samples,file="samples_BG2019_dd_centered_time30_sim_nodd_ddinter_stoch.Rdata")
  }else{
    save(list.samples,file="samples_BG2019_dd_centered_time30_sim_nodd_ddinter_nostoch.Rdata")
  }#stoch
}#time30
