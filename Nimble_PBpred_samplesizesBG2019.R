...
DDconstants  <-  list(nyears=nyears,r.j.p=r.j.p,r.j.v=r.j.v,
...
DDmodel$setData(list(marray.j.p=marray.j.p, 
                     marray.a.p=marray.a.p,
                     N.obs.p=N.obs.p, 
                     fledg.obs.p=fledg.obs.p,
                     marray.j.v=marray.j.v,
                     marray.a.v=marray.a.v,
                     r.a.p=r.a.p, r.a.v=r.a.v, 
                     N.obs.v=N.obs.v, fledg.obs.v=fledg.obs.v))
list.base <- list(dd.phi.p=-0.01,dd.fledg.rate.v=-0.005,mu.phi.p=c(0.5,qlogis(0.7)),
                  p.p=0.7,p.v=0.7,mu.fledg.rate.v=2,N1rec.p=N1rec.p,N1ad.p=N1ad.p,
                  N1rec.v=N1rec.v,N1ad.v=N1ad.v)
dependencies.base <- c(c("dd.phi.p","dd.fledg.rate.v","p.p","p.v",
                         "mu.phi.p","mu.phi.v","mu.fledg.rate.p","mu.fledg.rate.v",
                         "N1rec.v","N1rec.p","N1ad.v","N1ad.p"))
if(STOCH){
  if(DD_INTER){
    DDinits <- c(list(mu.fledg.rate.p=0,mu.phi.v=c(0.5,qlogis(0.6)),
                      sigma.phi.p=c(0.1,0.1),sigma.phi.v=c(0.1,0.1),
    ...
    nodesToSim  <-  DDmodel$getDependencies(c("one","sigma.phi.p","sigma.phi.v",
                                              "sigma.fledg.rate.p","sigma.fledg.rate.v",
                                              "dd.phi.v","dd.fledg.rate.p",dependencies.base),
                                            self = F, downstream = T)
  }else{
    DDinits <- c(list(mu.fledg.rate.p=0 + 0.004 * 101,mu.phi.v=c(0.5-0.025 * 21,qlogis(0.6)),
                      sigma.phi.p=c(0.1,0.1),sigma.phi.v=c(0.1,0.1),
  ...
  if(DD_INTER){
    ...
    DDinits <- c(list(mu.fledg.rate.p=0 + 0.004 * 101,
                      mu.phi.v=c(0.5-0.025 * 21,qlogis(0.6))),list.base)
...
for (i in 1:100){
  ...
  list.simul[[i]] <- list(N.rec.p=cDDmodel$N.rec.p,N.ad.p=cDDmodel$N.ad.p,
                          ...
                          marray.j.p=cDDmodel$marray.j.p,
                          marray.a.p=cDDmodel$marray.a.p,
                          r.a.p=cDDmodel$r.a.p, 
                          N.rec.v=cDDmodel$N.rec.v,
                          ...
                          marray.a.v=cDDmodel$marray.a.v,
                          r.a.v=cDDmodel$r.a.v)
  print(i)  
}
if(TIME10){
  if(DD_INTER){
    if(STOCH){
      save(list.simul,file="data/simul_BG2019_dd_time10n100_ddinter_stoch.Rdata")
    }else{
      save(list.simul,file="data/simul_BG2019_dd_time10n100_ddinter_nostoch.Rdata")}
  }else{
    if(STOCH){
      save(list.simul,file="data/simul_BG2019_dd_time10n100_noddinter_stoch.Rdata")}
    else{
      save(list.simul,file="data/simul_BG2019_dd_time10n100_noddinter_nostoch.Rdata")
    }
  }
}else{
  if(DD_INTER){
    if(STOCH){
      save(list.simul,file="data/simul_BG2019_dd_time30n20_ddinter_stoch.Rdata")
    }else{
      save(list.simul,file="data/simul_BG2019_dd_time30n20_ddinter_nostoch.Rdata")}
  }else{
    if(STOCH){
      save(list.simul,file="data/simul_BG2019_dd_time30n20_noddinter_stoch.Rdata")}
    else{
      save(list.simul,file="data/simul_BG2019_dd_time30n20_noddinter_nostoch.Rdata")
    }
  }#nodd
}#time30
if (DD_INTER) {
...
#configure the MCMC
monitor.base <- c("dd.phi.p","dd.fledg.rate.v","dd.phi.v","dd.fledg.rate.p",
                  "N.p","N.v","N.ad.p","N.ad.v",
...
for (i in 2:100){
  ...
  #Run the MCMC
  list.samples[[i]] <- runMCMC(cDDmcmc,niter=niter,nburnin=nburnin,thin=thin,nchains=nchains,setSeed=T)
  ##save posterior samples
  if(TIME10){
    if(TRUE_DD_INTER){
      if(STOCH){
        save(list.samples,file="data/samples_BG2019_dd_time10n100_ddinter_stoch.Rdata")
      }else{
        save(list.samples,file="data/samples_BG2019_dd_time10n100_ddinter_nostoch.Rdata")}
    }else{
      if(STOCH){
        save(list.samples,file="data/samples_BG2019_dd_time10n100_noddinter_stoch.Rdata")}
      else{
        save(list.samples,file="data/samples_BG2019_dd_time10n100_noddinter_nostoch.Rdata")
      }
    }
  }else{
    if(TRUE_DD_INTER){
      if(STOCH){
        save(list.samples,file="data/samples_BG2019_dd_time30n20_ddinter_stoch.Rdata")
      }else{
        save(list.samples,file="data/samples_BG2019_dd_time30n20_ddinter_nostoch.Rdata")}
    }else{
      if(STOCH){
        save(list.samples,file="data/samples_BG2019_dd_time30n20_noddinter_stoch.Rdata")}
      else{
        save(list.samples,file="data/samples_BG2019_dd_time30n20_noddinter_nostoch.Rdata")
      }
    }#nodd
  }#time30
  print(i)
